...
Node* deleteNode(Node* root, int key){
    Node* dNode = getNode(root, key);
    Node* replNode = replaceNode(dNode);

    //printf("dNodeP %d\n", dNode->key);
    //printf("replace Node %d\n", replNode->key);

    //delete node is leaf and red 
    if (dNode->c == 'R' && dNode->left == NULL && dNode->right == NULL){
        if (dNode->parent->left == dNode) dNode->parent->left = NULL;
        else dNode->parent->right = NULL;
    }

    else if (replNode != NULL && dNode->c == 'B' && ((dNode->left == NULL && dNode->right != NULL) || (dNode->left != NULL && dNode->right == NULL))){  
        if (replNode->parent->left == replNode) replNode->parent->left == NULL;
        else replNode->parent->right = NULL;

        if (dNode->parent->left == dNode){
            replNode->c = 'B';
            replNode->parent = dNode->parent;
            dNode->parent->left = replNode;
        }
        else{
            replNode->c = 'B';
            replNode->parent = dNode->parent;
            dNode->parent->right = replNode;
        }
    }

    else if (dNode->c == 'B' && (replNode == NULL || replNode->c == 'B')){
        printf("Entrou aqui\n");
        Node* sibling = NULL; 
        if (dNode->parent->left == dNode){
            sibling = dNode->parent->right;
            printf("Sibling c: %d", sibling->key);
            
            if (sibling == NULL || sibling->c == 'B'){
                printf("Sibling c: %c\n", sibling->c);
                if (sibling->left != NULL && sibling->right != NULL && (sibling->right->c == 'R' || sibling->left->c == 'R')){
        ...
        printf("N entrou em nada\n");
    }
    free(dNode);
    //printf("sNodeK: %d sNodeP: %d  sNodeL: %p sNodeR: %p\n", sNode->key, sNode->parent->key, sNode->left, sNode->right);
    return root;
}
